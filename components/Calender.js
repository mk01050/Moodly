...
export default function Calender(props) {
  ...
  function handleIncrementMonth(val) {
    ...
    if(numericMonth + val< 0 ){
      //set the month value = 11 and decrement month
      setSelectedYear(curr=>curr-1)
      setSelectedMonth(monthArr[monthArr.length-1])
    }else if(numericMonth+val >11){
      //set month val to 0 and increment month
      setSelectedYear(curr=>curr+1)
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex mx-auto gap-4 items-center ">
        <button onClick={()=>{handleIncrementMonth(-1)}} className="text-indigo-600 text-lg sm:text-xl duration-200 hover:opacity-60 auto" ><i className="fa-solid fa-circle-chevron-left"></i></button>
        <p className={"text-center capitalize textGradient  "+ fugaz.className}>{selectedMonth}, {selectedYear}</p>
        <button onClick={()=>{handleIncrementMonth(1)}} className="text-indigo-600 text-lg sm:text-xl duration-200 hover:opacity-60 auto"><i className="fa-solid fa-circle-chevron-right"></i></button>
      </div>
        ...
        return (
          <div key={rowIndex} className="grid grid-cols-7 gap-1">
            {dayList.map((dayOfWeek, dayOfWeekIndex) => {
              ...
              let isToday = dayIndex === now.getDate() && numericMonth === now.getMonth() ;
